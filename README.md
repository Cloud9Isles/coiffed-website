...
- `note.html`: reusable note detail page with stable query-string URLs
...
- Notes are managed in `notes-data.js`. Keep each slug unchanged after sharing its URL. Add the approved body as an array of paragraphs, then change `status` from `draft` to `published`. A note is treated as published only when both conditions are met.
